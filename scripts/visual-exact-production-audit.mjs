...
async function navigate(page, url) {
  ...
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    try {
      ...
      lastError = error;
      console.log(
        `Navegação ${url} (${attempt}/5): ${error instanceof Error ? error.message : String(error)}`,
      );
...
function reportMarkdown(results) {
  const lines = [
    ...
    "",
    "| Ambiente | Viewport | Estado | Overflow | Shell literal | Mapa | Embrapa | Águas | Exploração | Skip link | Navegação móvel | Rodapé |",
    "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
  ];
  ...
  for (const result of results) {
    ...
    lines.push(
      `| ${result.target} | ${result.viewport.width}×${result.viewport.height} | ${result.error ? "erro" : "capturado"} | ${audit?.horizontalOverflow ?? "n/a"}px | ${audit?.hasExactShell ? "sim" : "não"} | ${audit?.hasMap ? "sim" : "não"} | ${audit?.hasObservation ? "sim" : "não"} | ${audit?.hasWater ? "sim" : "não"} | ${audit?.hasExplore ? "sim" : "não"} | ${audit?.skipLinkVisible ? "sim" : "não"} | ${audit?.mobileNavigationVisible === null ? "n/a" : audit?.mobileNavigationVisible ? "sim" : "não"} | ${audit?.hasFooter ? "sim" : "não"} |`,
    );
  }
...
async function auditPage(page, target, viewport) {
  ...
  if (target.strict) {
    ...
    if (!audit.skipLinkFocused || !audit.skipLinkVisible) {
      failures.push("skip link sem foco visível");
    }
...
for (const target of targets) {
  for (const viewport of viewports) {
    ...
    const htmlFile = `${target.name}--${viewport.name}.html`;
    ...
    try {
      ...
      await writeFile(path.join(outputDirectory, htmlFile), await page.content());
      results.push({
        target: target.name,
        url: target.url,
        viewport,
      ...
      await page
        ...
        .catch(() => undefined);
      await writeFile(path.join(outputDirectory, htmlFile), await page.content().catch(() => ""));
      results.push({
        target: target.name,
        url: target.url,
        viewport,
...
for (const result of results) {
  console.log(
    `${result.target} ${result.viewport.width}×${result.viewport.height}: ${JSON.stringify(result.audit)} error=${result.error ?? "nenhum"}`,
  );
}
