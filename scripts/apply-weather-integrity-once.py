...
path = Path("src/production/components/home-editorial-dashboard.tsx")
text = path.read_text()

if 'type RainLevel = "unknown"' in text:
    print("Transformação já aplicada.")
    raise SystemExit(0)
...
def replace_once(old: str, new: str) -> None:
    global text
    if old not in text:
        raise RuntimeError(f"Trecho não encontrado: {old[:120]!r}")
    text = text.replace(old, new, 1)


def replace_pattern(pattern: str, replacement: str) -> None:
    global text
    text, count = re.subn(pattern, replacement, text, count=1, flags=re.S)
    if count != 1:
        raise RuntimeError(f"Bloco não encontrado: {pattern[:120]!r}")
...
replace_pattern(
    r"function rainReading\(value: number\) \{.*?\n\}\n\nfunction formatNumber",
...
replace_pattern(
    r"  const peakHour = hourly\.reduce<\(typeof hourly\)\[number\] \| null>\(.*?\n  const forecastWindow =",
...
replace_once("{today?.max ?? weather.current.temperature}°", '{today ? `${today.max}°` : "—"}')
replace_once("{today?.min ?? weather.current.temperature}°", '{today ? `${today.min}°` : "—"}')
replace_once(
    "{today?.rainChance ?? 0}%",
...
replace_once(
    ...
    "className={`rain-${rainReading(peakHour?.precipitation ?? null).level}`}",
)
replace_once(
    "{peakHour?.precipitation ?? 0}%",
    '{peakHour?.precipitation === null || !peakHour ? "Não informada" : `${peakHour.precipitation}%`}',
)
replace_once(
    "{strongestHourlyGust} km/h",
    '{strongestHourlyGust === null ? "Não informada" : `${strongestHourlyGust} km/h`}',
)

replace_pattern(
    r"            const className = \[\n\s+`rain-\$\{rain\.level\}`,\n\s+index === 0 \? \"is-current\" : \"\",\n\s+isPeak \? \"is-rain-peak\" : \"\",\n\s+\]",
    '''            const className = [
              `rain-${rain.level}`,
              isPeak ? "is-rain-peak" : "",
            ]''',
...
replace_once(
    ...
    '<span className="home-hourly-story__wind">{hour.windGust === null ? "Rajada não informada" : `Rajada de até ${hour.windGust} km/h`}</span>',
)

replace_once(
    "              const isRainiest = day.rainChance === rainiestDayChance && rain.chance >= 20;",
    "              const isRainiest = rainiestDayChance !== null && day.rainChance === rainiestDayChance && rain.chance !== null && rain.chance >= 20;",
)
replace_once(
...
replace_pattern(
    r"            A previsão indica o que pode acontecer\. A estação da Embrapa mostra as condições medidas\n\s+neste momento\.",
...
replace_once(
    ...
    '<span>{weather.current.feelsLike === null ? "Sensação não informada" : `Sensação de ${formatNumber(weather.current.feelsLike)} °C`}</span>',
)
replace_once(
    "<dd>{formatNumber(observation.current.humidity, 0)}%</dd>",
    '<dd>{weather.current.humidity === null ? "Não informada" : `${formatNumber(weather.current.humidity, 0)}%`}</dd>',
)
replace_once(
    "<dd>{formatNumber(observation.current.windSpeed)} km/h</dd>",
    '<dd>{weather.current.windSpeed === null ? "Não informado" : `${formatNumber(weather.current.windSpeed)} km/h`}</dd>',
)

path.write_text(text)
print(f"Transformação aplicada em {path}.")
