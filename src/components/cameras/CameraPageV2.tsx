...
> = {
  live: {
    label: "Ao vivo agora",
    title: "Transmissão ao vivo reconhecida",
    description: "O provedor informou que a transmissão está ao vivo nesta consulta.",
  },
  replay: {
    label: "Última transmissão",
    title: "Replay público disponível",
    description: "O player mostra a gravação pública mais recente encontrada no canal.",
  },
  configured: {
    label: "Player configurado",
    title: "Imagem disponível, estado não verificado",
    description: "Existe um player configurado, mas a integração não confirmou live ou replay.",
  },
  preparing: {
    label: "Em preparação",
    title: "Sem fonte pública estável",
    description: "O ponto está cadastrado, mas ainda não possui player público disponível.",
  },
};
...
function relativePublication(value: string | null) {
  ...
  return `Publicado há cerca de ${days} dias`;
}
  ...
  return (
    <section className="camera-v2-hero" aria-labelledby="camera-v2-hero-title">
      <div className="camera-v2-hero__content">
        ...
        <p>
          Veja céu, visibilidade, superfície da Lagoa e condições aparentes em locais específicos. Cada
          player informa se é live, replay, fonte configurada ou ponto ainda em preparação.
        </p>
      ...
      <aside className={`camera-v2-featured is-${featuredState}`} aria-label="Estado visual em destaque">
        ...
        <dl>
          ...
          <div><dt>Replay</dt><dd>{counts.replay}</dd></div>
          <div><dt>Configuradas</dt><dd>{counts.configured}</dd></div>
        </dl>
...
function CameraExplorerV2({ cameras }: { cameras: WeatherCamera[] }) {
  ...
  if (!selected) {
    return (
      <section className="camera-v2-empty" id="explorador-de-cameras" aria-labelledby="camera-v2-empty-title">
        ...
        <div>
          <span className="camera-v2-eyebrow">Nenhum ponto cadastrado</span>
          <h2 id="camera-v2-empty-title">Não há câmeras para exibir</h2>
          <p>O portal não cria imagens demonstrativas quando não existe uma fonte real cadastrada.</p>
        </div>
      </section>
    );
  }
  ...
  function selectCamera(camera: WeatherCamera) {
  ...
  return (
    <section className="camera-v2-explorer" id="explorador-de-cameras" aria-labelledby="camera-v2-explorer-title">
      <header className="camera-v2-section-heading">
        <div>
          <span className="camera-v2-eyebrow">Pontos cadastrados</span>
          <h2 id="camera-v2-explorer-title">Escolha o local e confira o estado do player</h2>
        </div>
        <p>
          A reprodução externa só é carregada após o clique. Trocar de câmera fecha o player anterior e
          preserva a identificação do estado e da procedência.
        </p>
      </header>
        ...
        <div className={`camera-v2-frame is-${selectedState}`}>
          {canPlay ? (
            ...
            <div className="camera-v2-placeholder" role="status">
              <VideoOff aria-hidden="true" />
              <strong>Player ainda não disponível</strong>
              <p>O ponto permanece cadastrado sem simular imagem ou transmissão.</p>
            </div>
          )}

          <div className="camera-v2-frame__status">
            <span><SelectedIcon aria-hidden="true" />{stateCopy[selectedState].label}</span>
            <small>{selected.provider ?? "Provedor não informado"}</small>
          </div>
        </div>

        <aside className="camera-v2-details" aria-label="Detalhes do ponto selecionado" aria-live="polite">
          <span className="camera-v2-eyebrow">Ponto selecionado</span>
          <h3>{selected.name}</h3>
          ...
          <dl>
            ...
            {selected.publishedAt ? (
              <div><dt>Publicação encontrada</dt><dd><Clock3 aria-hidden="true" />{formatDateTime(selected.publishedAt)} · {relativePublication(selected.publishedAt)}</dd></div>
            ) : null}
          </dl>

          {selected.streamTitle ? (
            <div className="camera-v2-details__title"><span>Título do provedor</span><strong>{selected.streamTitle}</strong></div>
          ) : null}

          {selected.publicUrl ? (
            <a href={selected.publicUrl} target="_blank" rel="noopener noreferrer" aria-label={`Abrir ${selected.name} no provedor externo, em nova aba`}>
...
export function CameraPageV2({ cameraData, weather }: CameraPageProps) {
  ...
  const hourly12 = weather.weather.hourly.slice(0, 12);
  const hourly24 = weather.weather.hourly.slice(0, 24);
  const maxRainChance = hourly12
  ...
  return (
    <div className="camera-v2-page">
      ...
      <nav className="camera-v2-chapters" aria-label="Capítulos das câmeras de Pelotas">
        <a href="#estado-das-cameras"><span>01</span><strong>Estado</strong><small>Live, replay ou configuração</small></a>
        <a href="#explorador-de-cameras"><span>02</span><strong>Câmeras</strong><small>Escolha o ponto</small></a>
        <a href="#contexto-meteorologico"><span>03</span><strong>Contexto</strong><small>Medição e previsão</small></a>
        <a href="#como-interpretar-imagens"><span>04</span><strong>Interpretação</strong><small>O que a imagem mostra</small></a>
        <a href="#responsabilidade-das-fontes"><span>05</span><strong>Limites</strong><small>Continuidade e segurança</small></a>
      </nav>

      <section className="camera-v2-status" id="estado-das-cameras" aria-labelledby="camera-v2-status-title">
        <div>
          <span className="camera-v2-eyebrow">Estado na última consulta</span>
          <h2 id="camera-v2-status-title">{counts.live ? `${counts.live} transmissão ao vivo` : "Nenhuma live confirmada agora"}</h2>
          <p>
            {cameraData.warning ?? "A integração respondeu sem aviso de indisponibilidade para o ponto principal."}
          </p>
        </div>
        <dl>
          <div><dt>Ao vivo</dt><dd>{counts.live}</dd></div>
          <div><dt>Replay</dt><dd>{counts.replay}</dd></div>
          <div><dt>Player sem estado verificado</dt><dd>{counts.configured}</dd></div>
          <div><dt>Em preparação</dt><dd>{counts.preparing}</dd></div>
        </dl>
        <footer><Info aria-hidden="true" />Consulta em {formatDateTime(cameraData.source.fetchedAt)} · {cameraData.source.name}</footer>
      </section>

      <CameraExplorerV2 cameras={cameraData.cameras} />

      <section className="camera-v2-weather" id="contexto-meteorologico" aria-labelledby="camera-v2-weather-title">
        <header className="camera-v2-section-heading">
          <div>
            <span className="camera-v2-eyebrow">Dados complementares</span>
            <h2 id="camera-v2-weather-title">A imagem e os dados respondem perguntas diferentes</h2>
          </div>
          <p>
            A câmera ajuda a observar o enquadramento. Temperatura atual vem da condição consolidada;
            chuva e rajada futuras são previsões e não podem ser inferidas diretamente do vídeo.
          </p>
        </header>
        <div className="camera-v2-weather__grid">
          <article><Thermometer aria-hidden="true" /><span>Temperatura consolidada</span><strong>{formatNumber(current?.temperature, " °C", 1)}</strong><small>{temperatureSource ? `Fonte do campo: ${sourceNames[temperatureSource]}` : "Fonte do campo não informada"}</small></article>
          <article><CloudRain aria-hidden="true" /><span>Maior chance de chuva</span><strong>{formatNumber(maxRainChance, "%")}</strong><small>Previsão nas próximas 12 horas</small></article>
          <article><Wind aria-hidden="true" /><span>Maior rajada prevista</span><strong>{formatNumber(maxGust, " km/h")}</strong><small>Previsão nas próximas 24 horas</small></article>
          <article><Gauge aria-hidden="true" /><span>Condição consolidada</span><strong>{current?.condition ?? "Em atualização"}</strong><small>Não é uma leitura produzida pela câmera</small></article>
        </div>
      </section>
      ...
      <section className="camera-v2-guidance" id="como-interpretar-imagens" aria-labelledby="camera-v2-guidance-title">
        <header className="camera-v2-section-heading">
          <div>
            <span className="camera-v2-eyebrow">Interpretação responsável</span>
            <h2 id="camera-v2-guidance-title">O que observar antes de concluir algo pela imagem</h2>
          </div>
          <p>
            Lente molhada, reflexos, horário, iluminação e enquadramento podem alterar a percepção. Uma
            imagem não mede temperatura, vento, volume de chuva ou nível da água.
          </p>
        </header>
        <div className="camera-v2-guidance__grid">
          <article><Eye aria-hidden="true" /><span>01</span><h3>Visibilidade</h3><p>Horizonte oculto pode indicar neblina, nuvem baixa ou precipitação, mas também pode resultar de posição e qualidade da lente.</p></article>
          <article><CloudRain aria-hidden="true" /><span>02</span><h3>Chuva aparente</h3><p>Gotas e superfície molhada ajudam na percepção local, mas não informam intensidade nem abrangência no município.</p></article>
          <article><Wind aria-hidden="true" /><span>03</span><h3>Vento aparente</h3><p>Movimento de árvores ou da Lagoa oferece contexto visual, porém não substitui anemômetro nem previsão de rajadas.</p></article>
          <article><Clock3 aria-hidden="true" /><span>04</span><h3>Atualidade</h3><p>Confirme se o estado é live, replay ou player sem verificação antes de interpretar a cena como condição presente.</p></article>
        </div>
      </section>
      ...
      <section className="camera-v2-responsibility" id="responsabilidade-das-fontes" aria-labelledby="camera-v2-responsibility-title">
        ...
        <div>
          <span className="camera-v2-eyebrow">Limites operacionais</span>
          <h2 id="camera-v2-responsibility-title">O portal não controla a continuidade das transmissões</h2>
          <p>
            Players, imagens e títulos pertencem aos responsáveis indicados em cada ponto. Uma fonte
            externa pode sair do ar, mudar de endereço, bloquear incorporação ou publicar replay sem aviso.
            Em situação de risco, prevalecem alertas oficiais e orientações das autoridades.
          </p>
        </div>
        <ShieldCheck aria-hidden="true" />
      </section>

      <section className="camera-v2-actions" aria-label="Ações relacionadas às câmeras">
        <div><span className="camera-v2-eyebrow">Compare as fontes</span><h2>Complete a observação visual com radar, medição e alertas</h2></div>
        <div>
