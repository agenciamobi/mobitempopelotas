...
function configuredRadarArea() {
  const area = readServerEnvironment("REDEMET_RADAR_AREA");
  return area && /^[a-z0-9_-]{1,20}$/i.test(area) ? area : "cn";
}

function configuredRadarProduct() {
  const product = readServerEnvironment("REDEMET_RADAR_PRODUCT");
  return product && /^[a-z0-9_-]{1,40}$/i.test(product) ? product : "maxcappi";
}
...
function normalizeImageFrames(raw: RawImageFrame[], fallbackBounds: RedemetBounds | null) {
...
function extractAnimationLabels(value: unknown) {
  const animation = findProperty(value, "anima");
  return Array.isArray(animation)
    ? animation.map(asString).filter((item): item is string => Boolean(item))
    : [];
}
...
function collectStormPoints(value: unknown) {
  ...
  if (record) {
    const latitude = asNumber(record.la ?? record.lat ?? record.latitude ?? record.y);
    ...
    if (
      ...
      if (distanceKm(PELOTAS_COORDINATES, point) <= STORM_RADIUS_KM) points.push(point);
    }
...
function extractStormFrames(value: unknown) {
  ...
  return rawFrames.map<RedemetStormFrame>((rawFrame, frameIndex) => {
    ...
    return {
      ...
      observedAt: parseDate(rawLabel)?.toISOString() ?? null,
      points: collectStormPoints(rawFrame),
    };
  });
}

export async function fetchRedemetRadar(frameCount = 10): Promise<RedemetImageLayerResponse> {
  const framesRequested = clampFrameCount(frameCount, 15, 10);
  const radarProduct = configuredRadarProduct();
  const product = `Radar meteorológico de Canguçu — ${radarProduct}`;
  ...
  try {
    const payload = await fetchRedemet(`produtos/radar/${encodeURIComponent(radarProduct)}`, {
      area: configuredRadarArea(),
    ...
    console.error("[redemet/radar] Consulta indisponível", {
...
export async function fetchRedemetSatellite(
  ...
  try {
    const payload = await fetchRedemet(`produtos/satelite/${type}`, {
      anima: framesRequested,
    });
    const frames = normalizeImageFrames(
      collectImageFrames(payload),
    ...
    console.error("[redemet/satellite] Consulta indisponível", {
      message: error instanceof Error ? error.message : "Falha desconhecida",
    });
...
export async function fetchRedemetStorms(frameCount = 20): Promise<RedemetStormLayerResponse> {
  ...
  try {
    const payload = await fetchRedemet("produtos/stsc", { anima: framesRequested });
