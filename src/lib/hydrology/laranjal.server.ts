import type { LaranjalLevelData, LaranjalLevelPoint } from "./laranjal.types";
...
async function getPublicAccessToken() {
  ...
  if (!response.ok) {
    throw new Error(`Autenticação pública respondeu com HTTP ${response.status}.`);
  }
...
export async function fetchLaranjalLevel(): Promise<LaranjalLevelData> {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Leituras do Laranjal responderam com HTTP ${response.status}.`);
    }
