...
import type { SafetyBanner } from "@/production/lib/safety-banners";
    ...
    >
      ...
      <div className="safety-alert-banner__actions">
        ...
        <button
          type="button"
          onClick={dismiss}
          aria-label="Ocultar esta orientação durante a sessão"
        >
          <span aria-hidden="true">×</span>
        </button>
      </div>
  ...
  return (
    ...
    >
      <div className="section-heading">
        ...
        <p>
          Escolha os canais adequados para sua rotina. O cadastro é gratuito e complementa os avisos
          exibidos pelo portal.
        </p>
      </div>
      ...
      <div className="safety-channels__grid">
        {banners.map((banner) => (
          <article
            className={`safety-channel-card safety-channel-card--${banner.tone}`}
            key={banner.id}
          >
