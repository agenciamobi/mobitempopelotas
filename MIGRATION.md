...
1. concluir e integrar os contratos de acessibilidade e segurança sem regressão no tema editorial;
2. conferir e aplicar as migrations no Supabase oficial, regenerar tipos e validar RLS com isolamento entre contas;
3. validar Google OAuth, cookies SSR, exportação e exclusão em preview e produção;
4. configurar VAPID e schedulers definitivos, com logs, alertas e testes reais de navegador;
5. executar auditoria WCAG 2.2 AA, Core Web Vitals, segurança, SEO de corte, domínio e plano de rollback;
6. remover o legado somente após evidência de paridade funcional e estabilidade em produção.
